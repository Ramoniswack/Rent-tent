...
export default function EditTripPage() {
  ...
  useEffect(() => {
    const fetchTripData = async () => {
      try {
        ...
        if (trip.imageUrl) {
          setCoverPhotoUrl(trip.imageUrl);
        }
        
        if (trip.lat && trip.lng) {
          setSelectedLocation({ lat: trip.lat, lng: trip.lng });
          setMapCenter([trip.lat, trip.lng]);
        }
      } catch (err: any) {
        console.error('Error fetching trip:', err);
        setError('Failed to load trip data. You may not have permission to edit this trip.');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await tripAPI.update(tripId, tripData);
      router.push(`/trips/${tripId}`);
    } catch (err: any) {
      setError(err.message || 'Failed to update trip. Try again?');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (fetchingTrip) {
    return (
      <div className="min-h-screen bg-[#f5f8f7] dark:bg-[#0b1713] flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-[#f5f8f7] dark:bg-[#0b1713] selection:bg-emerald-500 selection:text-white">
      ...
      <main className="max-w-4xl mx-auto px-4 py-12 md:py-20 animate-fadeIn">
        ...
        <button
          onClick={() => router.back()}
          className="mb-6 flex items-center gap-2 text-black dark:text-white hover:text-emerald-500 dark:hover:text-emerald-400 transition-colors font-bold"
        >
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="lg:w-1/3 space-y-6">
            ...
            <h1 className="text-4xl font-black text-black dark:text-white tracking-tight leading-tight">
              Edit Your <br />
              <span className="text-emerald-500 text-5xl">Journey</span>
            </h1>
            <p className="text-black dark:text-white font-medium leading-relaxed">
              Update your trip details, adjust dates, or change your destination.
            </p>
          ...
          <div className="lg:w-2/3">
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 md:p-12 shadow-2xl shadow-emerald-900/5 border border-white dark:border-white/5 relative overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                >
                  {loading ? <Loader2 className="animate-spin" /> : 'Update Trip'}
                </button>
              </form>
            </div>
          </div>
