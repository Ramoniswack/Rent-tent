  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#f5f8f7] dark:bg-[#0f231d]">
        ...
        <main className="mx-auto w-full max-w-[1440px] px-4 md:px-20 py-12">
          {/* Hero Title Section */}
          <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <h1 className="text-5xl md:text-[48px] font-black text-[#0f172a] dark:text-white mb-2">
                Your Journeys.
              </h1>
            ...
            <button 
              ...
              className="bg-[#059467] hover:bg-[#047853] text-white px-8 py-4 rounded-full font-bold text-base shadow-lg shadow-[#059467]/20 flex items-center gap-2 transition-all transform hover:-translate-y-1"
            >
            ...
            <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
              ...
              <div className="bg-[#059467]/5 p-1.5 rounded-full flex w-full lg:w-auto">
                <button 
                  ...
                  className={`flex-1 lg:min-w-[160px] py-3 px-6 rounded-full font-bold text-sm transition-all ${
                    activeTab === 'all'
              ...
              <div className="flex items-center gap-3 w-full lg:max-w-xl">
                <div className="relative flex-1 group">
                  <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 dark:text-slate-500 group-focus-within:text-[#059467] transition-colors" />
                  <input
                    className="w-full pl-12 pr-4 py-4 bg-white dark:bg-slate-800 border-none rounded-full shadow-sm ring-1 ring-slate-200 dark:ring-slate-700 focus:ring-2 focus:ring-[#059467] outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500 text-[#0f172a] dark:text-white"
                    placeholder="Search trips, locations, or itineraries..."
                    type="text"
                    value={searchQuery}
                ...
                <div className="relative filter-menu-container">
                  <button 
                    ...
                    className={`bg-white dark:bg-slate-800 p-4 rounded-full shadow-sm ring-1 transition-all flex items-center justify-center ${
                      statusFilter !== 'all' 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {filteredTrips.map((trip) => {
              ...
              return (
                <div 
                  key={trip._id}
                  className="trip-card group bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-xl shadow-slate-900/5 dark:shadow-black/20 overflow-hidden flex flex-col transition-all hover:shadow-2xl hover:shadow-[#059467]/10 cursor-pointer"
                ...
                >
                  <div className="relative aspect-[4/3] overflow-hidden">
                    ...
                    {/* Status Badge */}
                    <div className="absolute top-6 left-6 bg-white/70 dark:bg-slate-900/70 backdrop-blur-md px-4 py-1.5 rounded-full flex items-center gap-2">
                    ...
                    {/* Public Badge */}
                    {activeTab === 'public' && (
                      <div className="absolute top-6 right-6 bg-blue-500/90 backdrop-blur-md px-4 py-1.5 rounded-full flex items-center gap-2">
                        <svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM4.332 8.027a6.012 6.012 0 011.912-2.706C6.512 5.73 6.974 6 7.5 6A1.5 1.5 0 019 7.5V8a2 2 0 004 0 2 2 0 011.523-1.943A5.977 5.977 0 0116 10c0 .34-.028.675-.083 1H15a2 2 0 00-2 2v2.197A5.973 5.973 0 0110 16v-2a2 2 0 00-2-2 2 2 0 01-2-2 2 2 0 00-1.668-1.973z" clipRule="evenodd" />
                        </svg>
                        <span className="text-xs font-bold text-white uppercase tracking-wider">Public</span>
                      </div>
                    )}

                    {/* Hover Actions - Only for user's trips */}
                    {activeTab !== 'public' && (
                      <div className="action-overlay opacity-0 absolute inset-0 bg-[#0f172a]/20 backdrop-blur-[2px] flex items-center justify-center gap-4 transition-opacity duration-300">
                        <button 
                          onClick={(e) => {
                            e.stopPropagation();
                  ...
                  <div className="p-8 flex-1 flex flex-col">
                    <h3 className="text-2xl font-black text-[#0f172a] dark:text-white mb-4 group-hover:text-[#059467] transition-colors">
                      {trip.title}
                    </h3>
                    <div className="flex flex-col gap-2 mb-8">
                      <div className="flex items-center gap-2 text-slate-600 dark:text-slate-400">
                        ...
                        <span className="text-sm font-medium">{trip.destination}, {trip.country}</span>
                      </div>
                      <div className="flex items-center gap-2 text-slate-600 dark:text-slate-400">
                        <Calendar className="w-5 h-5" />
                        <span className="text-sm font-medium">{formatDateRange(trip.startDate, trip.endDate)}</span>
                      </div>
                    </div>

                    <div className="mt-auto flex items-center justify-between border-t border-[#059467]/5 dark:border-slate-700 pt-6">
                      <div className="flex -space-x-3">
                        {displayMembers.map((member, idx) => (
                          <div key={`${trip._id}-member-${member._id}-${idx}`} className="relative">
                            {member.profilePicture ? (
                              <img
                                alt={member.name}
                                className="w-10 h-10 rounded-full border-2 border-white dark:border-slate-800 ring-1 ring-[#059467]/20 object-cover"
                                src={member.profilePicture}
                              />
                            ) : (
                              <div className="w-10 h-10 rounded-full border-2 border-white dark:border-slate-800 ring-1 ring-[#059467]/20 bg-gradient-to-br from-[#059467] to-[#047853] flex items-center justify-center">
                            ...
                            {idx === 0 && (
                              <div className="absolute -top-2 -right-1 bg-yellow-400 text-white rounded-full p-0.5 shadow-sm border border-white dark:border-slate-800">
                        ...
                        {remainingCount > 0 && (
                          <div className="w-10 h-10 rounded-full bg-[#059467]/10 dark:bg-slate-700 border-2 border-white dark:border-slate-800 flex items-center justify-center text-[10px] font-bold text-[#059467] dark:text-white ring-1 ring-[#059467]/20">
                            +{remainingCount}
                          </div>
                        )}
                      </div>
                      <div className="text-[#059467] font-bold text-sm flex items-center gap-1 group/link">
                        Details
