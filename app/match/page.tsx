...
import { 
  Heart, X, MapPin, Users, Sparkles, Info, Zap, MessageCircle, 
...
const TravelMatch: React.FC = () => {
  ...
  const fetchInteractedUsers = async () => {
    try {
      ...
      // Only set past interactions on initial load so array doesn't shrink during swiping
      setPastInteractedIds(result.interactedUserIds || []);
    } catch (error) {
  ...
  const checkProfile = async () => {
    try {
      ...
      setProfileComplete(!!(profile.name && profile.profilePicture && profile.username));
    } catch (error) {
  ...
  // NEW: Programmatic physical swipe out animation
  const swipeOut = async (direction: 'left' | 'right') => {
    ...
    await animate(x, targetX, { 
      ...
      stiffness: 400, 
      damping: 25,
      restDelta: 1 // Resolves the promise slightly earlier for snappiness
    });

    setCurrentIndex(prev => prev + 1);
    x.set(0); // Instantly reset position for the next card below it
  };
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      await animationPromise; // Wait for card to finish flying off
      
      if (result.matched && result.matchedUser) {
        setMatches(prev => [...prev, result.matchedUser]);
    ...
    return (
      <PageLayout>
        <div className="flex-grow flex items-center justify-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-[2rem] shadow-xl p-8 border border-white/60 dark:border-slate-700/50 w-full">
  ...
  return (
    <PageLayout>
      <div className="relative flex-grow flex flex-col justify-center h-[calc(100vh-280px)] min-h-[450px]">
        {hasSeenAll ? (
          ...
          <div className="relative w-full h-full">
            <AnimatePresence mode="popLayout">
              ...
              {currentIndex + 2 < filteredProfiles.length && (
                <motion.div key={`bg-2-${currentIndex}`} initial={{ scale: 0.85, opacity: 0, y: 30 }} animate={{ scale: 0.90, opacity: 0.4, y: 20 }} exit={{ scale: 0.85, opacity: 0 }} transition={{ duration: 0.3 }} className="absolute inset-0 bg-white dark:bg-slate-800 rounded-[2rem] shadow-md border border-slate-200 dark:border-slate-700/50" />
              )}
              {currentIndex + 1 < filteredProfiles.length && (
                <motion.div key={`bg-1-${currentIndex}`} initial={{ scale: 0.90, opacity: 0, y: 20 }} animate={{ scale: 0.95, opacity: 0.8, y: 10 }} exit={{ scale: 0.90, opacity: 0 }} transition={{ duration: 0.3 }} className="absolute inset-0 bg-white dark:bg-slate-800 rounded-[2rem] shadow-lg border border-slate-200 dark:border-slate-700/50" />
              )}
              ...
              <motion.div
                ...
                // Since our custom swipeOut logic handles the exit animation manually, we bypass AnimatePresence for the top card to avoid flickering
              ...
              >
                <img src={currentProfile.profilePicture || 'https://i.pravatar.cc/800?img=' + Math.floor(Math.random() * 70)} alt={currentProfile.name} className="w-full h-full object-cover pointer-events-none" />
      ...
      {showMatch && lastMatch && userProfile && (
        <MatchSuccess
          isOpen={showMatch}
          onClose={() => { setShowMatch(false); setLastMatch(null); }}
          onSendMessage={() => { setShowMatch(false); setLastMatch(null); router.push(`/messages?user=${lastMatch._id || lastMatch.id}`); }}
          matchedUser={{ name: lastMatch.name, profilePicture: lastMatch.profilePicture }}
          currentUser={{ name: userProfile.name, profilePicture: userProfile.profilePicture }}
        />
      )}
